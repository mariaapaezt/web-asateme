...
import { propagarGanadoresPlayoffs } from "../componentes/liga/playoffLogic.js";    
...
export class CargaVista {
  ...
  cambiarLiga(liga) {
    ...
    if (
      this.selector &&
      typeof this.selector.actualizarBotonesLiga === "function"
    ) {
  ...
  async cambiarFecha(fecha) {
    ...
    // ⚡ Si la fecha elegida es de Playoffs, forzamos recarga de Supabase para tener las llaves actualizadas
    if (String(fecha).startsWith("PLAYOFF_")) {
      await this.cargarDatos();
    }
  ...
  obtenerNumerosFechas() {
    ...
    return [...new Set(partidosDeLiga.map((p) => Number(p.fecha_numero)))]
  ...
  poblarFechas(resetearFecha = false) {
    ...
    if (resetearFecha) {
      this.fechaActual =
        numerosFecha.length > 0 ? numerosFecha[0] : "PLAYOFF_CUARTOS";
    }
  ...
  async cargarDatos() {
    ...
    const partidosPlayoffs = (resPlayoffs.data || []).map((p) => ({
      ...
      score_local:
        p.puntos_equipo_1 !== null && p.puntos_equipo_1 !== undefined
          ? p.puntos_equipo_1
          : null,
      score_visitante:
        p.puntos_equipo_2 !== null && p.puntos_equipo_2 !== undefined
          ? p.puntos_equipo_2
          : null,
    }));
    ...
    (resEquipos.data || []).forEach(
      (eq) => (this.equiposMapa[eq.id] = eq.nombre || `Club ${eq.id}`),
    );
  ...
  async alSeleccionarPartido(partidoId) {
    ...
    const partido = this.partidosCache.find(
      (p) => String(p.id) === String(partidoId),
    );
    if (!partido) return;

    const estadoUpper = String(partido.estado || "")
      .toUpperCase()
      .trim();
    ...
    if (estaCerrado) {
      this.actualizarTotales(
        partido.score_local ?? 0,
        partido.score_visitante ?? 0,
      );
      alert(
        "🔒 Esta serie se encuentra cerrada y no permite la carga de nuevos resultados.",
      );
      return;
    }
    ...
    if (!partido.local_id || !partido.visitante_id) {
      alert(
        "⏳ Este cruce de playoffs aún no tiene los dos equipos definidos.",
      );
      return;
    }

    const nomLoc =
      this.equiposMapa[partido.local_id] || `Local (${partido.local_id})`;
    const nomVis =
      this.equiposMapa[partido.visitante_id] ||
      `Visitante (${partido.visitante_id})`;

    if (document.getElementById("txt-nombre-local"))
      document.getElementById("txt-nombre-local").textContent = nomLoc;
    if (document.getElementById("txt-nombre-visitante"))
      document.getElementById("txt-nombre-visitante").textContent = nomVis;
    if (document.getElementById("nombre-club-local"))
      document.getElementById("nombre-club-local").textContent = nomLoc;
    if (document.getElementById("nombre-club-visitante"))
      document.getElementById("nombre-club-visitante").textContent = nomVis;
  ...
  actualizarTotales(scoreL, scoreV) {
    if (document.getElementById("score-local-input"))
      document.getElementById("score-local-input").value = scoreL;
    if (document.getElementById("score-visitante-input"))
      document.getElementById("score-visitante-input").value = scoreV;
    if (document.getElementById("marcador-parcial-txt"))
      document.getElementById("marcador-parcial-txt").textContent =
        `${scoreL} - ${scoreV}`;
  }
  ...
  actualizarEstiloBotonesLiga() {
    ...
    if (this.ligaActual === "LIGA_A") {
      ...
      btnA.classList.remove(...inactivo);
      btnB.classList.add(...inactivo);
      btnB.classList.remove(...activo);
    } else {
      btnB.classList.add(...activo);
      btnB.classList.remove(...inactivo);
      btnA.classList.add(...inactivo);
  ...
  actualizarDesplegablePartidos() {
    ...
    if (esPlayoff) {
      ...
      if (this.fechaActual === "PLAYOFF_CUARTOS") {
        filtrados = listaPlayoffs.filter(
          (p) => p.fase === "CUARTOS" || p.codigo_llave?.startsWith("QF"),
        );
      } else if (this.fechaActual === "PLAYOFF_SEMI") {
        filtrados = listaPlayoffs.filter(
          (p) => p.fase === "SEMIFINAL" || p.codigo_llave?.startsWith("SF"),
        );
      } else if (this.fechaActual === "PLAYOFF_FINAL") {
        filtrados = listaPlayoffs.filter(
          (p) => p.fase === "FINAL" || p.codigo_llave === "FINAL",
        );
      }
    } else {
      // Filtro por Fase Regular
      filtrados = this.partidosCache.filter(
        (p) =>
          !p.esPlayoff &&
          (p.liga || "").toUpperCase() === this.ligaActual.toUpperCase() &&
  ...
  async guardar(e) {
    ...
    const partidoSeleccionado = this.partidosCache.find(
      (p) => String(p.id) === String(this.partidoId),
    );
    ...
    // ----------------------------------------------------
    const esPlayoff = Boolean(
      partidoSeleccionado.esPlayoff ||
      String(this.fechaActual).startsWith("PLAYOFF_"),
    );
    ...
    try {
      ...
      if (errConsulta) {
        console.error(
          "💥 Error devuelto por Supabase al consultar estado:",
          errConsulta,
        );
        alert(
          `⚠️ Error al verificar el estado del partido en el servidor.\nDetalle: ${errConsulta.message || "Error de conexión/permisos"}`,
        );
        return false;
      }

      if (partidoBD) {
        const estadoReal = String(partidoBD.estado || "")
          .toUpperCase()
          .trim();
        ...
        if (
          ...
          alert(
            "🔒 Esta serie ya fue enviada previamente y se encuentra cerrada.",
          );
          return false;
        }
      }
      ...
      alert(
        `⚠️ No se pudo verificar el estado actual del partido: ${errEstado.message || errEstado}`,
      );
      return false;
    }
    ...
    const chkVisitanteWO =
      document.getElementById("wo-visitante-check")?.checked;
    ...
    if (!esWalkover) {
      ...
      for (let i = 1; i <= 5; i++) {
        ...
        if (!part || !part.terminado) {
          alert(
            `⚠️ Planilla incompleta. El Partido ${i} no registra un ganador reglamentario.`,
          );
          return false;
        }
        if (
          !part.local1 ||
          !part.vis1 ||
          (part.modalidad === "DOBLES" && (!part.local2 || !part.vis2))
        ) {
    ...
    // ----------------------------------------------------
    const scoreLocal =
      parseInt(document.getElementById("score-local-input")?.value) || 0;
    const scoreVisitante =
      parseInt(document.getElementById("score-visitante-input")?.value) || 0;

    const btnEnviar =
      document.querySelector('#form-envio-planilla button[type="submit"]') ||
      document.getElementById("btn-enviar-planilla");
    ...
    try {
      ...
      const { data: equiposValidados, error: errToken } = await window.supabase
        ...
        .select("*")
        .or(
          `id.eq.${partidoSeleccionado.local_id},id.eq.${partidoSeleccionado.visitante_id}`,
        )
      ...
      const localNombre =
        this.equiposMapa[partidoSeleccionado.local_id] ||
        `Local_${partidoSeleccionado.local_id}`;
      const visitanteNombre =
        this.equiposMapa[partidoSeleccionado.visitante_id] ||
        `Visitante_${partidoSeleccionado.visitante_id}`;

      const { urlPlanilla, urlPagoLoc, urlPagoVis } =
        await subirComprobantesPartido({
      ...
      if (!esWalkover) {
        ...
        const tablaDetalles = esPlayoff
          ? "liga_playoffs_detalles"
          : "fixture_detalles";
        ...
        for (let i = 1; i <= 5; i++) {
          ...
          filasDetalle.push({
            ...
            sets_local: Array.isArray(part.setsL)
              ? part.setsL
              : [0, 0, 0, 0, 0],
            sets_visitante: Array.isArray(part.setsV)
              ? part.setsV
              : [0, 0, 0, 0, 0],
      ...
      if (esPlayoff) {
        ...
        let puntosEq2 = scoreVisitante;

        // ⚡ Ajuste por Walkover (WO): Asigna 5 a 0 automáticamente
        if (chkLocalWO) {
          ganadorId = partidoSeleccionado.visitante_id;
          puntosEq1 = 0;
          puntosEq2 = 5;
        } else if (chkVisitanteWO) {
          ganadorId = partidoSeleccionado.local_id;
          puntosEq1 = 5;
          puntosEq2 = 0;
        } else {
        ...
        const { error: errUpdatePlayoff } = await window.supabase
          .from("liga_playoffs")
          .update({
            puntos_equipo_1: puntosEq1,
            puntos_equipo_2: puntosEq2,
        ...
        // ⚡ 1. Propagar ganadores a la siguiente ronda en Supabase
        await propagarGanadoresPlayoffs(this.ligaActual, window.supabase);
