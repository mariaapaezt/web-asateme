...
export const TarjetaEquipo = {
    ...
    renderGrid(equipos, jugadores, equipoSeleccionadoId) {
        ...
        return equipos.map(equipo => {
            const esSeleccionado = String(equipo.id) === String(equipoSeleccionadoId);
            const cantJugadores = (jugadores || []).filter(j => String(j.equipo_id) === String(equipo.id)).length;

            return `
                <button onclick="window.seleccionarEquipoDetalle('${equipo.id}')" 
                        class="p-4 rounded-xl border transition-all text-left flex items-center justify-between gap-3 cursor-pointer ${esSeleccionado
    ...
    renderJugadores(jugadores, detallesPartidos, maxFechas) {
        ...
        return jugadores.map(jugador => {
            ...
            const inicial = jugador.nombre ? jugador.nombre.charAt(0).toUpperCase() : 'J';

            // Cálculo de presencia / fechas jugadas
            const partidosJugados = (detallesPartidos || []).filter(d =>
            ...
            const fechasUnicas = new Set(partidosJugados.map(p => p.partido_id)).size;
            const porcentajePresencia = maxFechas > 0 ? Math.round((fechasUnicas / maxFechas) * 100) : 0;
                    ...
                    <div class="flex items-center gap-3 min-w-0">
                        ...
                        <div class="min-w-0">
                            ...
                            <span class="text-[10px] text-gray-400 block mt-0.5">
                                Fechas: ${fechasUnicas} / ${maxFechas || 12}
                            </span>
                        </div>
                    </div>
