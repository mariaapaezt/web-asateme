 ...
 */
export async function inicializarPlayoffsEnSupabase(
  ligaId = "LIGA_A",
  ...
  if (tablaPosiciones.length < 6) {
    console.warn(
      `⚠️ No hay suficientes equipos (${tablaPosiciones.length}/6) en ${ligaId} para armar playoffs.`,
    );
    return false;
  }
 ...
 */
export async function verificarEInicializarPlayoffs(
  ligaId = "LIGA_A",
  supabaseClient = window.supabase,
) {
  ...
  if (partidosLiga.length === 0) {
    return {
      ok: false,
      mensaje: "No hay partidos registrados en la Fase Regular.",
    };
  }

  const partidosPendientes = partidosLiga.filter(
    (p) => !p.estado || p.estado.toLowerCase() !== "finalizado",
  );

  if (partidosPendientes.length > 0) {
    console.warn(
      `⏳ Fase regular incompleta. Quedan ${partidosPendientes.length} partido(s) pendientes.`,
    );
    return {
      ok: false,
  ...
  return {
    ok: false,
    mensaje: "Error al generar la llave de Playoffs en Supabase.",
  };
}
 ...
 */
export async function propagarGanadoresPlayoffs(
  ligaId = "LIGA_A",
  supabaseClient = window.supabase,
) {
  if (!supabaseClient) return;

  // 1. Obtener el estado actual de los playoffs directamente desde Supabase
  ...
  if (
    qf2?.ganador_id &&
    sf1 &&
    String(sf1.equipo_2_id) !== String(qf2.ganador_id)
  ) {
  ...
  if (
    qf1?.ganador_id &&
    sf2 &&
    String(sf2.equipo_2_id) !== String(qf1.ganador_id)
  ) {
    await supabaseClient
      .from("liga_playoffs")
  ...
  if (
    sf1?.ganador_id &&
    finalMatch &&
    String(finalMatch.equipo_1_id) !== String(sf1.ganador_id)
  ) {
    await supabaseClient
      .from("liga_playoffs")
  ...
  if (
    sf2?.ganador_id &&
    finalMatch &&
    String(finalMatch.equipo_2_id) !== String(sf2.ganador_id)
  ) {
    await supabaseClient
      .from("liga_playoffs")
  ...
  // ⚡ 2. Refrescar la caché interna en memoria
  await ligaState.init(supabaseClient);
}
